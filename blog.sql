...
CREATE TABLE IF NOT EXISTS `blog_auth` (
  ...
  `upadted_by` VARCHAR(100) DEFAULT '' COMMENT 'Username updated by',
...
CREATE TABLE IF NOT EXISTS `blog_article` (
    `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
    `author` VARCHAR(100) DEFAULT 'anonymous' COMMENT 'author',
    ...
    `upadted_by` VARCHAR(100) DEFAULT '' COMMENT 'Article updated by',
    `created_at` datetime COMMENT 'Created time',
    `updated_at` datetime COMMENT 'updated time',
    `deleted_at` datetime COMMENT 'deleted time',
    PRIMARY KEY (id)
